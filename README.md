...
Jpeg는 촬영이 가능했지만 압축된 이미지라
RGB값을 추출할려면 디코딩을 해야하는데 메모리가 많이 필요하다 확인했습니다
그래서 라즈베리파이에 일단 JPEG그대로 전송하고
거기서 켜진 segment를 확인하여 어떤 문자/숫자인지 계산후 다시 ESP32로 값을 넘겨줬습니다

라즈베리파이에서 다 처리할 수 도 있었지만
알람의 끄라고하는 것도 켜라고하는 것도 하는 첫 시작점을 ESP32로 통일하고 싶어서 조금 비효율적으로 작성했습니다
